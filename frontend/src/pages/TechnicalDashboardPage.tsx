...
useEffect(() => {
    const fetchWorks = async () => {
        try {
            ...
            const endpoint = `${API_URL}/api/reports/status/${selectedStatus}`;
    ...
    const startTransfer = async (id: string) => {
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(err?.message || "No se pudo iniciar el traslado");
            }

            // Quitar el trabajo de la lista actual para reflejar el cambio inmediato
            setWorks((prev) => prev.filter((w) => w.id !== id));
        } catch (error: any) {
            console.error(error);
            alert(error?.message || "Error al iniciar traslado");
        }
    };
                            ...
                            >
                                ...
                                <div className="technician-work-content">
                                    ...
                                    <div className= "technician-work-footer">
                                        ...
                                        {work.status === "PRIORITIZED" && (
                                            <button
                                                onClick={() => startTransfer(work.id)}
                                                className="technician-transfer-button"
                                            >
                                                Iniciar traslado
                                            </button>
                                        )}
