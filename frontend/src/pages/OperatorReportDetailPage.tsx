    ...
    useEffect(() => {
        const fetchReport = async () => {
            try {
                ...
                setTargetDate(data.targetDate ? data.targetDate.slice(0, 10) : "");
    ...
    const handlePrioritize = async (event: React.FormEvent) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || "No se pudo priorizar el reporte.");
            }
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 bg-white rounded-3xl border shadow-sm p-8">
                        <div className="flex items-start justify-between gap-6 mb-8">
                            <div>
                                <h1 className="text-5xl font-bold text-[#03152E]">
                            ...
                            <div className="flex flex-col gap-2 items-end">
                                ...
                                {report.priority && (
                                    <span className={`px-5 py-3 rounded-full font-semibold border ${priorityClass(report.priority)}`}>
                        ...
                        <img
                            src={report.evidences?.[0]?.imageUrl || "https://placehold.co/1200x600?text=Sin+Evidencia"}
                        ...
                        <div className="mt-10 grid grid-cols-1 lg:grid-cols-2 gap-10">
                            <div>
                                ...
                                <p className="text-lg text-gray-600 leading-relaxed">{report.description}</p>
                            </div>
                            ...
                            <div>
                                <div className="mb-10">
                                    ...
                                    <div className="bg-gray-50 rounded-2xl p-6">
                                        <p className="text-gray-500 mb-2">Dirección registrada</p>
                    ...
                    <aside className="bg-white rounded-3xl border shadow-sm p-8 h-fit space-y-6">
                        {report.status === "REGISTERED" && (
                            <div className="space-y-4">
                                <h2 className="text-2xl font-bold text-[#03152E]">Evaluar reporte</h2>
                                <p className="text-gray-500 text-sm">Revisa la descripción, ubicación y evidencias antes de aprobar o rechazar.</p>
                                <hr />
                                <button
                        ...
                        {(report.status === "APPROVED" || report.status === "PRIORITIZED") && (
                            <form onSubmit={handlePrioritize} className="space-y-5">
                                <h2 className="text-2xl font-bold text-[#03152E]">Formulario de priorización</h2>
                                <hr />

                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-1">Impacto del problema</label>
                                    <select
                                        value={impact}
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-1">Tipo operativo</label>
                                    <input
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-1">Fecha objetivo</label>
                                    <input
                                        type="date"
                                        value={targetDate}
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-1">Justificación</label>
                                    <textarea
                                        value={justification}
                                ...
                                <button
                                    type="submit"
                                    disabled={submitting}
                                    className="w-full bg-blue-700 hover:bg-blue-800 disabled:bg-gray-400 text-white rounded-2xl p-4 text-xl font-semibold transition"
                                >
                        ...
                        {report.status !== "REGISTERED" && report.status !== "APPROVED" && report.status !== "PRIORITIZED" && (
                            <div>
                                <h2 className="text-2xl font-bold text-[#03152E]">Acciones no disponibles</h2>
                                <p className="text-gray-500 mt-2">Este reporte ya avanzó a otra etapa del flujo.</p>
                            </div>
                        )}

                        {report.priority && (
                            <div className="bg-gray-50 rounded-2xl p-5 space-y-3">
                                <h3 className="font-bold text-lg">Datos de priorización</h3>
                                <p><strong>Prioridad:</strong> {report.priority}</p>
                                {report.operationalType && <p><strong>Tipo operativo:</strong> {report.operationalType}</p>}
                                {report.targetDate && <p><strong>Fecha objetivo:</strong> {new Date(report.targetDate).toLocaleDateString()}</p>}
                                {report.justification && <p><strong>Justificación:</strong> {report.justification}</p>}
                            </div>
                        )}
                    </aside>
                </div>
