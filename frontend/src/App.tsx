...
const reportAdapter = {
  toCreateReportDTO: (formData: ReportFormValues, userId: string): CreateReportDTO => ({
...
export default function App() {
  ...
  const canSubmit = useMemo(() => {
    return formData.title.trim() && formData.description.trim() && formData.location.trim();
  }, [formData]);
    ...
    <div className="app">
      <div className="container">
        <header className="header">
          <p className="kicker">Módulo de reportes</p>
          <h1 className="title">Registro de problemas del distrito</h1>
        </header>

        <section className="card">
          <h2>Registrar reporte</h2>
          <p>Campos obligatorios: título, descripción y ubicación.</p>
        ...
        <section className="card">
          ...
          <p>Aquí puedes ver los reportes que se van registrando.</p>

          <div className="list">
            {reports.length === 0 ? (
              <div className="empty">Aún no hay reportes creados desde este frontend.</div>
            ) : (
              reports.map((report) => (
