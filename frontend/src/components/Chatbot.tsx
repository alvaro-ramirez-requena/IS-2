...
function getPageContext(pathname: string): { page: string; pageId?: string } {
  ...
  if (pathname === "/my-reports")
    return {
      page: "my-reports",
      pageId: localStorage.getItem("userId") || undefined,
    };
  if (pathname === "/operator") return { page: "operator" };
  if (pathname.startsWith("/operator/report/"))
    return { page: "operator-detail", pageId: pathname.split("/").pop() };
  if (pathname.startsWith("/fieldwork/"))
    return { page: "fieldwork", pageId: pathname.split("/").pop() };
...
function getWelcomeMessage(page: string, role: string): string {
  if (role === "OPERATOR") {
    if (page === "operator")
      return "Hola, soy tu asistente municipal. ¿Quieres saber cuántos reportes están pendientes o cuáles son los más urgentes?";
    if (page === "operator-detail")
      return "Estoy revisando este reporte contigo. ¿Tienes dudas sobre aprobarlo o rechazarlo?";
  }
  if (role === "TECHNICIAN") {
    if (page === "fieldwork")
      return "Estoy aquí para ayudarte con este trabajo en campo. ¿Qué necesitas saber?";
  }
  if (page === "my-reports")
    return "Puedo ayudarte a entender el estado de tus reportes. ¿Qué quieres saber?";
