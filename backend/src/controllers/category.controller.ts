...
export class CategoryController {
  ...
  static async getActive(req: Request, res: Response) {
    try {
      ...
      return res.status(400).json({
        message: error.message || "Error al obtener categorías activas.",
      });
    }
  }
  ...
  static async deactivate(req: Request, res: Response) {
    try {
      const category = await categoryService.deactivate(String(req.params.id));

      return res.json(category);
    } catch (error: any) {
      return res.status(400).json({
        message: error.message || "Error al desactivar categoría.",
      });
    }
  }
  ...
  static async activate(req: Request, res: Response) {
    try {
      const category = await categoryService.activate(String(req.params.id));

      return res.json(category);
    } catch (error: any) {
      return res.status(400).json({
        message: error.message || "Error al activar categoría.",
      });
    }
  }
}
