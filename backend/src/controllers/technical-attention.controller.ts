...
const technicalAttentionService =
  new TechnicalAttentionService();
...
export class TechnicalAttentionController {
  static async create(
    ...
    try {
      ...
      return res.status(400).json({
        message:
          error.message ||
          "Error al registrar la atención técnica.",
      });
    }
  }
  ...
  static async getByReport(
    ...
    try {
      ...
      const attentions =
        await technicalAttentionService.getByReport(
          reportId
        );
      ...
      return res.status(400).json({
        message:
          error.message ||
          "Error al obtener atenciones técnicas.",
      });
    }
  }

  static async getLatestByReport(
    req: Request,
    res: Response
  ) {
    try {
      ...
      const attention =
        await technicalAttentionService.getLatestByReport(
          reportId
        );
      ...
      return res.status(400).json({
        message:
          error.message ||
          "Error al obtener la atención técnica.",
      });
    }
  }
}